...
### ✅ P2 — 业务基础（已完成 2026-09-08）
- 40 张业务表 DDL + 老系统 3038 行存量数据（mysql2sqlite.py 自动转换）
- 通用 CRUD（参数化路由 + PRAGMA 运行时表结构 + 逻辑删除 + GSP 留档保护）
- 工作流：两级审批 8 表 + 证照变更、采购计划单级审批 + 转采购单、采购入库五步流
- dashboard 统计接真实业务表

### ✅ P3 — 业务增强（已完成 2026-09-09，端到端 61/61 通过）
- **销售出库六级流**（销售经理→质管员→库管员→质量负责人→销售总监→已出库）
  - 库存预占/释放/终审精确批次扣减 + inventory_log 流水
  - 财务自动记账（finance_record 收入）+ audit_trail 六级审批轨迹 JSON
  - 已出库 GSP 留档不可改；驳回后编辑重提自动回到起点
- **产品验收五步流**（外观检查→数量核对→质量检验→综合判定→已完成）
  - 任一环节"不合格"合规拦截 → 不合格待处理 → 支持重置重新验收
  - 效期分布统计端点
- **审批通知收件箱**：通知推送（按用户/角色/广播）+ 未读数 + 逐条/全部已读；前端 MainLayout 铃铛 30s 轮询
- **报表中心**：7 类 Excel(.xlsx) 导出 + 采购/库存/销售/财务汇总 + 近 6 个月月度趋势（前端 /report 页面）
- **验收资料 PDF 上传下载**：仅 PDF ≤20MB、随业务单据（biz_type/biz_id）留档、支持 `?token=` 浏览器直开
- 顺手修复：P2 通用 CRUD / 两级审批 / 五步流路由的 `req.params` 参数名不匹配 bug（此前服务从未跑起来过未暴露）

**端到端验证**：`bash server/scripts/e2e-test.sh`（61 项断言覆盖 P1/P2/P3 全链路）
